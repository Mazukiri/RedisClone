...
func cmdHELLO(args []string) []byte {
	return Encode([]string{"server", "memkv", "proto", "2", "version", "1.0.0"}, false)
}

func cmdCLIENT(args []string) []byte {
...
func EvalAndResponse(cmd *MemKVCmd, c io.ReadWriter) error {
	...
	case "HELLO":
		res = cmdHELLO(cmd.Args)
	case "CLIENT":
		res = cmdCLIENT(cmd.Args)
	case "SET":
		res = cmdSET(cmd.Args)
	case "GET":
		res = cmdGET(cmd.Args)
