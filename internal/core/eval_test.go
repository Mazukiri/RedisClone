...
func TestCmdZADD(t *testing.T) {
	delete(zsetStore, "myzset")
	res, err := Decode(cmdZADD([]string{"myzset", "1", "one"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 1, res)

	res, err = Decode(cmdZADD([]string{"myzset", "1", "uno"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 1, res)

	res, err = Decode(cmdZADD([]string{"myzset", "2", "two", "3", "three"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 2, res)

	zset := zsetStore["myzset"]
	assert.EqualValues(t, 4, zset.Len())
}

func TestCmdZRANK(t *testing.T) {
	delete(zsetStore, "myzset")
	cmdZADD([]string{"myzset", "1", "one", "2", "two", "3", "three"})

	res, err := Decode(cmdZRANK([]string{"myzset", "three"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 2, res)

	res, err = Decode(cmdZRANK([]string{"myzset", "four"})) // Not exist
	assert.Nil(t, err)
	assert.Nil(t, res)
}
...
func TestCmdZREM(t *testing.T) {
	...
	res, err := Decode(cmdZREM([]string{"myzset", "two"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 1, res)

	zset := zsetStore["myzset"]
	assert.EqualValues(t, 2, zset.Len())

	res, err = Decode(cmdZREM([]string{"myzset", "nonexistent"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 0, res)
}
...
func TestCmdZSCORE(t *testing.T) {
	delete(zsetStore, "myzset")
	cmdZADD([]string{"myzset", "1.5", "one"})

	res, err := Decode(cmdZSCORE([]string{"myzset", "one"}))
	assert.Nil(t, err)
	assert.EqualValues(t, "1.500000", res)

	res, err = Decode(cmdZSCORE([]string{"myzset", "missing"}))
	assert.Nil(t, err)
	assert.Nil(t, res)
}

func TestCmdZCARD(t *testing.T) {
	delete(zsetStore, "myzset")
	cmdZADD([]string{"myzset", "1", "one", "2", "two"})

	res, err := Decode(cmdZCARD([]string{"myzset"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 2, res)
}

func TestCmdStringOps(t *testing.T) {
	// Setup
	dictStore = data_structure.CreateDict()

	// SET
	res, err := Decode(cmdSET([]string{"mykey", "helloworld"}))
	assert.Nil(t, err)
	assert.EqualValues(t, "OK", res)

	// GET
	res, err = Decode(cmdGET([]string{"mykey"}))
	assert.Nil(t, err)
	assert.EqualValues(t, "helloworld", res)

	// EXPIRE
	res, err = Decode(cmdEXPIRE([]string{"mykey", "100"})) // 100 seconds
	assert.Nil(t, err)
	assert.EqualValues(t, 1, res)

	// TTL
	res, err = Decode(cmdTTL([]string{"mykey"}))
	assert.Nil(t, err)
	// TTL should be <= 100 and > 0
	ttlVal := res.(int64)
	assert.True(t, ttlVal > 0 && ttlVal <= 100)

	// DEL
	res, err = Decode(cmdDEL([]string{"mykey"}))
	assert.Nil(t, err)
	assert.EqualValues(t, 1, res)
